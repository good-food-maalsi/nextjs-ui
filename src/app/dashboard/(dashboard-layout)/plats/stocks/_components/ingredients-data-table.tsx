...
interface ColumnsProps<TData> {
  onEdit: (ingredient: TData) => void;
  onDelete: (ingredient: TData) => void;
}

interface IngredientsDataTableProps<TData, TValue> {
  columns:
...
export function IngredientsDataTable<TData, TValue>({
  ...
  onEdit,
  onDelete,
}: IngredientsDataTableProps<TData, TValue>) {
  const safeData = data ?? [];
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const fournisseur = String(row.getValue("fournisseur") || "").toLowerCase();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <Input
          placeholder="Rechercher (nom, description, fournisseur)..."
          value={globalFilter}
            ...
            {isLoading ? (
              ...
              <TableRow>
                ...
                >
                  ...
                  >
                    {isError
                      ? "Une erreur est survenue lors du chargement des ingrédients."
