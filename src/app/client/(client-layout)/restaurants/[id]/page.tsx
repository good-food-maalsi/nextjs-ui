...
export default function RestaurantDetailPage({
  ...
  } = useCatalogContent(contentParams, {
  ...
  const franchiseMenus =
    hasSearch && searchResult ? searchResult.menus : (contentResult?.menus ?? []);
  const franchiseDishes =
    hasSearch && searchResult ? searchResult.dishes : (contentResult?.dishes ?? []);
    ...
    hasSearch && debouncedSearchQuery.trim().length > 0
      ...
      : franchiseMenus.filter(
          (menu) =>
            ...
            menu.name.toLowerCase().includes(debouncedSearchQuery.toLowerCase()) ||
            (menu.description
              ?.toLowerCase()
  ...
  if (isError || !franchise) {
    return (
      <div className="h-screen flex items-center justify-center flex-col gap-4">
        ...
        <button
          onClick={() => router.push("/client/restaurants")}
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredDishes.map((dish) => (
                <DishGridItem key={dish.id} dish={dish} />
              ))}
            </div>
          </section>
        )}
