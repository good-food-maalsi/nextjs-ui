...
export function PopularCategories() {
  ...
  return (
    <section className="py-12">
      ...
      {isLoading && (
        <div className="text-black-400 text-sm py-8">Chargement des catégories...</div>
      )}
      {isError && (
        <div className="text-destructive text-sm py-8">Impossible de charger les catégories.</div>
      )}
      {!isLoading && !isError && categories.length === 0 && (
        <div className="text-black-400 text-sm py-8">Aucune catégorie pour le moment.</div>
      )}
      {!isLoading && !isError && categories.length > 0 && (
