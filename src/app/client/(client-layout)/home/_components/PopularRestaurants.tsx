...
export function PopularRestaurants() {
  ...
  return (
    <section className="py-12">
      ...
      {isLoading && (
        <div className="text-black-400 text-sm py-8">Chargement des franchises...</div>
      )}
      {isError && (
        <div className="text-destructive text-sm py-8">Impossible de charger les franchises.</div>
      )}
      {!isLoading && !isError && franchises.length === 0 && (
        <div className="text-black-400 text-sm py-8">Aucune franchise pour le moment.</div>
      )}
      {!isLoading && !isError && franchises.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {franchises.map((franchise, index) => (
            <Link
              key={franchise.id}
              href={`/client/restaurant/${franchise.id}`}
              className="group"
            >
              <div className="relative rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:scale-105">
                ...
                <div className="bg-primary-300 p-3 text-center">
                  ...
                  {franchise.city && (
                    <p className="text-xs text-black-400 mt-0.5">{franchise.city}</p>
                  )}
                </div>
              </div>
            </Link>
