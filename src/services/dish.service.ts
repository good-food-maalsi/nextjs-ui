...
async function update(id: string, data: UpdateDishInput): Promise<Dish> {
  const response = await catalogClient.dishes.update({
    params: { id },
    body: data,
  });
...
async function deleteDish(id: string): Promise<void> {
  const response = await catalogClient.dishes.delete({
    params: { id },
    body: {},
  });
...
async function getIngredients(dishId: string): Promise<DishIngredient[]> {
  const response = await catalogClient.dishIngredients.getAll({
    params: { id: dishId },
  });
  if (response.status !== 200)
    throw new Error("Failed to fetch dish ingredients");
  return response.body.data;
}
...
async function addIngredient(
  ...
  const response = await catalogClient.dishIngredients.add({
    params: { id: dishId },
    body: data,
  });
  if (response.status !== 201)
    throw new Error("Failed to add ingredient to dish");
  return response.body.data;
}
...
async function updateIngredient(
  ...
  if (response.status !== 200)
    throw new Error("Failed to update dish ingredient");
  return response.body.data;
}
...
async function removeIngredient(
  ...
  if (response.status !== 200)
    throw new Error("Failed to remove ingredient from dish");
}
